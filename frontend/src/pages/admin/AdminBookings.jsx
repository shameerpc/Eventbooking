  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      if (Array.isArray(res.data)) {
        // Case 1: API returns the array directly -> res.data = [...]
        data = res.data;
      } else if (res.data && Array.isArray(res.data.bookings)) {
        // Case 2: API returns an object -> res.data = { bookings: [...] }
              ...
              bookings.map((booking) => {
                ...
                return (
                  <tr key={booking._id}>
                    ...
                    <td style={styles.td}>
                      {/* Safety check: map seats if array, otherwise show IDs or count */}
                      {Array.isArray(booking.seats) 
                        ? booking.seats.map(s => s.seatNumber || s).join(', ') 
